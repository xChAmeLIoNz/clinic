...
function checkProduct($sku, $qty, $conn,$id_user)
{
    ...
    if ($conn->connect_error) {
        ...
        $sql = "SELECT * FROM export_mag WHERE sku='$sku' AND id_user='$id_user';";
        $result = $conn->query($sql);
        //se esiste nel cart, allora aggiorno il prodotto
        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $final_qty = $qty + $row['qty_neg'];
            if ($conn->query("UPDATE export_mag SET qty_neg='$final_qty' WHERE sku='$sku' AND id_user='$id_user';")) {
            ...
            if ($result->num_rows > 0) {
                ...
                if ($conn->query("INSERT INTO export_mag VALUES (DEFAULT, '$id', '$name', '$price', '$sku', '$qty', '$id_user');")) {
...
if (isset($_POST['data'])) {
    ...
    if ($sku != '' && $qty != '') {
        $data_array = checkProduct($sku, $qty, $conn, $id_user);
    }
    echo json_encode($data_array);
}


//CODE FOR editCartMag.php
if (isset($_POST['sku'], $_POST['qty'])) {
    ...
    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        //$final_qty = $qty + $row['qty_neg'];
        if ($conn->query("UPDATE export_mag SET qty_neg='$qty' WHERE sku='$sku' AND id_user='$id_user';")) {
            header("Location: cartMag.php");
            //echo "dentro update";
        }
    }
}
